...
def predict_track_metadata(model, metadata=[], output_layer=-1):
    try:
        ...
        if output_layer == -1:
            ...
            pred = get_activations(model, output_layer, patches_meta)[0][0]
    except Exception as e:
        pred = []
        print(str(e))
        print('Error predicting track')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Predict piano detection from a trained model.')
    parser.add_argument('-m', '--model_id', required=True, help='Model ID')
    parser.add_argument('-t', '--trained_tsv', default=common.DEFAULT_TRAINED_MODELS_FILE, help='Trained models TSV file')
    parser.add_argument('-f', '--test_file', default=TESTSET_FILE, help='Test file')
    parser.add_argument('-p', '--pred_dataset', default="", help='Prediction dataset')
    parser.add_argument('-s', '--spectro_folder', default="", help='Spectrogram folder')
    parser.add_argument('-n', '--set_name', default="test", help='Set name')
    parser.add_argument('-r', '--rnd_selection', action='store_true', help='Random selection')
    parser.add_argument('-o', '--output_layer', type=int, default=-1, help='Output layer')
    parser.add_argument('-w', '--with_patches', action='store_true', help='With patches')
    parser.add_argument('-df', '--data_file', default="", help='Data file')
    parser.add_argument('-di', '--data_index', default="", help='Data index')
