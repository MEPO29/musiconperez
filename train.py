...
from sklearn.metrics import roc_curve, auc, roc_auc_score, precision_score, recall_score, f1_score, accuracy_score, average_precision_score, r2_score
...
class Config(object):
    ...
    def __init__(self, params, normalize=False, whiten=True):
        ...
        self.norm = normalize
        self.whiten = whiten
        self.x_path = '%s_%s' % (params['dataset']['meta-suffix'],params['dataset']['dataset'])
...
def _magnitude(x):
    return tf.sqrt(tf.maximum(_squared_magnitude(x), np.finfo(x.dtype.as_numpy_dtype).tiny))

def cosine(x, y):
    x = x[:, :50]
    cosine_similarity = tf.reduce_sum(x * y, axis=-1) / (_magnitude(x) * _magnitude(y))
    cosine_distance = (1 - cosine_similarity) / 2
    cosine_distance = tf.clip_by_value(cosine_distance, 0.0, 1.0)
...
def load_data_preprocesed(params, X_path, Y_path, dataset, val_percent, test_percent, n_samples, with_metadata=False, only_metadata=False, metadata_source='rovi'):
    ...
    # index_train = open(common.DATASETS_DIR + '/items_index_train_' + dataset + '.tsv').read().splitlines()
    index_train = open(common.TRAINDATA_DIR+'/items_index_train_%s.tsv' % (dataset)).read().splitlines()
    ...
    if params['training']["val_from_file"]:
        ...
        Y_test = np.load(common.DATASETS_DIR+'/y_test_'+Y_path+'.npy')
...
def load_data_hf5_memory(params, val_percent, test_percent, y_path, id2gt, X_meta=None, val_from_file=False):
    if val_from_file:
        hdf5_file = common.PATCHES_DIR+"/patches_train_%s_%sx%s.hdf5" % (
            params['dataset']['dataset'], params['dataset']['npatches'], params['dataset']['window'])
        f = h5py.File(hdf5_file, "r")
        index_train = f["index"][:]
        index_train = np.delete(index_train, np.where(index_train == ""))
        N_train = index_train.shape[0]

        val_hdf5_file = common.PATCHES_DIR+"/patches_val_%s_%sx%s.hdf5" % (
            params['dataset']['dataset'], params['dataset']['npatches'], params['dataset']['window'])
        ...
        test_hdf5_file = common.PATCHES_DIR+"/patches_test_%s_%sx%s.hdf5" % (
            params['dataset']['dataset'], params['dataset']['npatches'], params['dataset']['window'])
...
def process(params, with_predict=True, with_eval=True):
    ...
    with_metadata = params['dataset'].get('with_metadata', False)
    only_metadata = params['dataset'].get('only_metadata', False)
    metadata_source = params['dataset'].get('meta-suffix', 'rovi')
    if with_metadata:
        if 'w2v' in metadata_source:
    ...
    if only_metadata:
        ...
        if 'meta-suffix2' in params['dataset']:
            X_train2, Y_train2, X_val2, Y_val2, X_test2, Y_test2 = \
                load_data_preprocesed(params, 'model_text_MuMu-albums', config.y_path, params['dataset']['dataset'], config.training_params["validation"],
        ...
        if 'meta-suffix3' in params['dataset']:
            X_train3, Y_train3, X_val3, Y_val3, X_test3, Y_test3 = \
                load_data_preprocesed(params, 'model_visual_MuMu-albums', config.y_path, params['dataset']['dataset'], config.training_params["validation"],
    ...
    if only_metadata:
        ...
        steps_per_epoch = (N_train - (N_train % config.training_params["n_minibatch"])) // config.training_params["n_minibatch"]
        epochs = model.fit(batch_block_generator(params, config.y_path, N_train, id2gt, X_meta, config.training_params["val_from_file"]),
    ...
    if test_shape[0] == Y_test.shape[0]:
        ...
        with open('train_results.txt', 'a+') as fw:
